...
export const FacturasRectificativasListado = ({ onEditar, onVer, onNuevo, modoVisual = "claro", abrirModalHistorialDocumento: abrirModalHistorialProp }) => {
  const {
    documentos,
    loading,
    paginacion,
  ...
  const ejecutarTransformacion = async () => {
    ...
    try {
      const tipoDocumentoMap = {
        'DUPLICAR': { endpoint: API_ENDPOINTS.facturasRectificativas, serie: 'FACTURA_RECTIFICATIVA' },
  ...
  return (
    ...
    >
      ...
      <ModalHistorialTransformaciones
        ...
        documento={documentoHistorial ? { ...documentoHistorial, tipo: 'FACTURA_RECTIFICATIVA' } : null}
