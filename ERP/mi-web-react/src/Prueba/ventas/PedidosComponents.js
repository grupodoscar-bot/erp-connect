...
export const PedidosListado = ({ onEditar, onVer, onNuevo, modoVisual = "claro", abrirModalHistorialDocumento: abrirModalHistorialProp }) => {
  const {
    documentos,
    loading,
    paginacion,
  ...
  const abrirModalTransformar = async (pedido) => {
  ...
  const ejecutarTransformacion = async () => {
    ...
    try {
      const tipoDocumentoMap = {
        'DUPLICAR': { endpoint: API_ENDPOINTS.pedidos, serie: 'PEDIDO' },
      ...
      const body = {
        tipoOrigen: 'PEDIDO',
        idOrigen: pedidoParaTransformar.id,
  ...
  return (
    ...
    >
      ...
      <ModalHistorialTransformaciones
        ...
        documento={documentoHistorial ? { ...documentoHistorial, tipo: 'PEDIDO' } : null}
