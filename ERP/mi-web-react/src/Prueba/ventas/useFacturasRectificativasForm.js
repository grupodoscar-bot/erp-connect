...
// Estados mínimos requeridos por el backend (Pendiente y Emitido son obligatorios)
...
export function useFacturasRectificativasForm(pestanaActiva = null, session = null) {
  ...
  const guardarFacturaRectificativa = useCallback(
    async (e, opciones = {}) => {
      ...
      try {
        ...
        const fechaModificada = formFacturaRectificativa.fecha !== formFacturaRectificativa.fechaOriginal;
        const fechaFormateada = fechaModificada 
          ? new Date(formFacturaRectificativa.fecha).toISOString()
  ...
  const cargarFacturaRectificativaParaEditar = useCallback(async (facturaRectificativa) => {
    ...
    try {
      ...
      const fechaLocal = facturaRectificativaCompleta.fecha 
        ? new Date(facturaRectificativaCompleta.fecha).toISOString().slice(0, 16)
