...
export const PresupuestosListado = ({ onEditar, onVer, onNuevo, modoVisual = "claro", abrirModalHistorialDocumento: abrirModalHistorialProp }) => {
  const {
    documentos,
    loading,
    paginacion,
  ...
  const abrirModalTransformar = async (presupuesto) => {
    ...
    setFechaTransformacion(fechaHora);
  ...
  const ejecutarTransformacion = async () => {
    ...
    try {
      const tipoDocumentoMap = {
        'DUPLICAR': { endpoint: API_ENDPOINTS.presupuestos, serie: 'PRESUPUESTO' },
        'ALBARAN': { endpoint: API_ENDPOINTS.albaranes, serie: 'ALBARAN_VENTA' },
        'FACTURA': { endpoint: API_ENDPOINTS.facturas, serie: 'FACTURA_VENTA' },
      ...
      const body = {
        ...
        idOrigen: presupuestoParaTransformar.id,
      ...
      const tipoLabel = tipoTransformacionSeleccionado === 'DUPLICAR' 
        ? 'Presupuesto duplicado' 
        : `${config.serie.replace(/_/g, ' ')} creado`;
      alert(`${tipoLabel}: ${resultado.numero}`);
  ...
  return (
    ...
    >
      ...
      <ModalHistorialTransformaciones
        ...
        documento={documentoHistorial ? { ...documentoHistorial, tipo: 'PRESUPUESTO' } : null}
                    ...
                    {proveedorSeleccionado ? (
                      ...
                      <div style={{ position: 'relative' }}>
                        <input
                          ...
                          onFocus={() => mostrarProveedores || busquedaProveedor.length >= 3}
                          style={{ width: '100%' }}
                        />
