...
const API_URL = "http://145.223.103.219:8080/albaranes";
...
export function useAlbaranes({ setMensaje, abrirPestana, cerrarPestana, pestanaActiva, session }) {
  ...
  const formAlbaran = useMemo(() => {
    if (!pestanaActiva) return formAlbaranInicial;
    return formulariosPorPestana[pestanaActiva] || formAlbaranInicial;
  }, [formulariosPorPestana, pestanaActiva]);
  
  // Función para actualizar el formulario de la pestaña actual
  const setFormAlbaran = useCallback((nuevoFormulario, pestanaId = null) => {
    ...
    setFormulariosPorPestana(prev => ({
      ...
      [idPestana]: typeof nuevoFormulario === 'function' 
        ? nuevoFormulario(prev[idPestana] || formAlbaranInicial)
        : nuevoFormulario
    }));
  }, [pestanaActiva]);
  ...
  const [estadoOptions, setEstadoOptions] = useState(ESTADOS_ALBARAN_PREDETERMINADOS);
    ...
    if (pestanaActiva && pestanaActiva.startsWith('albaran-nuevo')) {
      setFormulariosPorPestana(prev => {
        if (!prev[pestanaActiva]) {
          setEstadoOriginalAlbaran(ESTADOS_ALBARAN_PREDETERMINADOS[0]?.nombre || "Pendiente");
          return {
            ...prev,
            [pestanaActiva]: { ...formAlbaranInicial }
          };
        }
        return prev;
      });
  ...
  const asegurarEstadoEnOpciones = useCallback((estado) => {
    if (!estado) return;
    setEstadoOptions((prev) => {
      const existe = prev.some(e => e.nombre === estado);
      if (existe) return prev;
      return [...prev, { nombre: estado, colorClaro: "#FDE68A55", colorOscuro: "#92400E55" }];
    });
  }, []);
  ...
  const cargarConfiguracionVentas = useCallback(async () => {
    try {
      ...
      if (estados.length) {
        setEstadoOptions(normalizarEstados(estados));
      }
    } catch (err) {
  ...
  const sincronizarEstadosConfigurables = useCallback(
    (estados) => {
      const normalizados = normalizarEstados(estados);
      const base = normalizados.length ? normalizados : ESTADOS_ALBARAN_PREDETERMINADOS;
      const estadoActual = formAlbaranRef.current?.estado;
      const nombresBase = base.map(e => e.nombre);
      const opcionesFinales =
        estadoActual && !nombresBase.includes(estadoActual)
          ? [...base, { nombre: estadoActual, colorClaro: "#FDE68A55", colorOscuro: "#92400E55" }]
          : base;

      setEstadoOptions(opcionesFinales);
      setFormAlbaran((prev) => {
        const nombresOpciones = opcionesFinales.map(e => e.nombre);
        if (prev.estado) {
          if (nombresOpciones.includes(prev.estado)) {
            return prev;
          }
          // Mantener el estado actual aunque no exista en la lista para evitar saltos inesperados
          return { ...prev };
        }
        const nuevoEstado = opcionesFinales[0]?.nombre || ESTADOS_ALBARAN_PREDETERMINADOS[0].nombre;
        return {
          ...prev,
          estado: nuevoEstado,
        };
      ...
      setFiltroEstado((prev) => {
        if (!prev) return prev;
        const nombresOpciones = opcionesFinales.map(e => e.nombre);
        return nombresOpciones.includes(prev) ? prev : "";
      });
    },
    [setFiltroEstado]
  );

  const cargarEstadosConfiguracion = useCallback(async () => {
    try {
      const res = await fetch(API_URL_CONFIG_VENTAS);
      if (!res.ok) {
      ...
      const data = await res.json();
      sincronizarEstadosConfigurables(data?.estadosAlbaran || []);
      setPermitirVentaMultialmacen(data?.permitirVentaMultialmacen || false);
    } catch (err) {
      console.error("Error cargando estados configurables:", err);
      sincronizarEstadosConfigurables(ESTADOS_ALBARAN_PREDETERMINADOS);
    }
  }, [sincronizarEstadosConfigurables]);
  ...
  const cargarAlmacenes = useCallback(async () => {
    try {
      ...
      const data = await res.json();
      const almacenesActivos = (Array.isArray(data) ? data : data.content || []).filter(a => a.activo);
        ...
        const albaranesConOrigen = await Promise.all(resultado.map(async (albaran) => {
          ...
          try {
            ...
            if (resOrigen.ok) {
              ...
              return { ...albaran, origen: formatearTipo(dataOrigen.tipoOrigen), totalBaseSinImpuestos, totalIva };
            }
  ...
  useEffect(() => {
    ...
    const esPestanaAlbaranes = pestanaActiva.startsWith('albaran-nuevo') || 
  ...
  }, [paginaActual, itemsPorPagina, ordenarPor, ordenDireccion, busqueda, filtroEstado, filtroSerieId, filtroNumero, filtroFechaDesde, filtroFechaHasta, filtroImporteMin, filtroImporteMax, cargarAlbaranes, pestanaActiva]);
  ...
  const cargarTiposIva = useCallback(async () => {
    try {
      ...
      console.error("Error cargando tipos IVA:", err);
    }
  }, []);
  ...
  const cargarSeries = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${SERIES_API_URL}?${params.toString()}`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      ...
      setMensaje("No se pudieron cargar las series disponibles");
      return [];
    } finally {
  ...
  const guardarPreferenciaSerie = useCallback(async () => {
    if (!usuarioId) {
      setMensaje("No hay un usuario activo para guardar la preferencia");
      return;
    }
    if (!formAlbaran.serieId) {
      setMensaje("Selecciona una serie antes de guardarla como predeterminada");
      return;
    }
    ...
    try {
      const res = await fetch(SERIES_PREF_API_URL, {
        ...
        body: JSON.stringify({
          usuarioId,
          tipoDocumento: DOCUMENTO_SERIE_TIPO,
          serieId: parseInt(formAlbaran.serieId),
        }),
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      setSeriePreferidaUsuario(data);
      setMensaje("Serie guardada como predeterminada");
    } catch (err) {
      console.error("Error guardando preferencia de serie:", err);
      setMensaje("No se pudo guardar la preferencia de serie");
    } finally {
      setGuardandoPreferenciaSerie(false);
    }
  }, [usuarioId, formAlbaran.serieId, setMensaje]);
  ...
  // Cargar datos maestros solo cuando hay una pestaña de albaranes activa
  const [datosMaestrosCargados, setDatosMaestrosCargados] = useState(false);
  ...
  useEffect(() => {
    ...
    if (esPestanaAlbaranes && !datosMaestrosCargados) {
      setDatosMaestrosCargados(true);
      cargarClientes();
      cargarProductos();
      cargarTiposIva();
      cargarSeries();
  ...
  }, [pestanaActiva, datosMaestrosCargados, cargarClientes, cargarProductos, cargarTiposIva, cargarSeries, cargarAlmacenes, cargarConfiguracionVentas]);

  // Recargar datos maestros cuando la ventana recupera el foco
  useEffect(() => {
    const handleFocus = () => {
      cargarClientes();
      cargarProductos();
  ...
  }, [cargarClientes, cargarProductos, cargarTiposIva, cargarSeries, cargarAlmacenes, cargarConfiguracionVentas]);
  ...
  }, [pestanaActiva, cargarPreferenciaSerie, usuarioId]);
  ...
  // ========== TRANSFORMAR ==========
  const abrirModalTransformar = useCallback((albaran) => {
  ...
  useEffect(() => {
    ...
    const cargarSeriesPorTipo = async () => {
      ...
      try {
        ...
        console.log('URL de carga de series:', url);
  ...
  const ejecutarTransformacion = useCallback(async () => {
    ...
    try {
      // Usar endpoint genérico de transformación para DUPLICAR y ALBARAN
      if (tipoTransformacionSeleccionado === 'DUPLICAR' || tipoTransformacionSeleccionado === 'ALBARAN') {
        const payload = {
          tipoOrigen: 'ALBARAN',
        ...
        const res = await fetch(`${API_URL}/transformar`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const accion = tipoTransformacionSeleccionado === 'DUPLICAR' ? 'duplicado' : 'transformado';
        setMensaje(`Albarán ${creado.numero} ${accion} correctamente`);
        cerrarModalTransformar();
        cargarAlbaranes();
        return;
      }

      // Para otros tipos, usar endpoints específicos
      switch (tipoTransformacionSeleccionado) {
        case 'FACTURA': {
          const payload = {
            albaranId: albaranParaTransformar.id,
            serieId: serieSeleccionada ? parseInt(serieSeleccionada) : null,
            fecha: fechaTransformacion,
            estado: estadoTransformacion,
          };

          const res = await fetch(`http://145.223.103.219:8080/facturas/desde-albaran`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload)
          });
          ...
          setMensaje(`Factura ${factura.numero} creada correctamente`);
          cerrarModalTransformar();
          break;
        }
        ...
        case 'FACTURA_PROFORMA': {
          ...
          const res = await fetch(`http://145.223.103.219:8080/facturas-proforma/desde-albaran`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload)
          });
          ...
          const factura = await res.json();
          setMensaje(`Factura Proforma ${factura.numero} creada correctamente`);
          cerrarModalTransformar();
          break;
        }
        ...
        case 'FACTURA_RECTIFICATIVA': {
          ...
          const res = await fetch(`http://145.223.103.219:8080/facturas-rectificativas/desde-albaran`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload)
          });
          ...
          const factura = await res.json();
          setMensaje(`Factura Rectificativa ${factura.numero} creada correctamente`);
          cerrarModalTransformar();
          break;
        }
        ...
        case 'PEDIDO': {
          ...
          const res = await fetch(`http://145.223.103.219:8080/pedidos/desde-albaran`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload)
          });
          ...
          setMensaje(`Pedido ${pedido.numero} creado correctamente`);
          cerrarModalTransformar();
          break;
        }
        ...
        case 'PRESUPUESTO': {
          ...
          const res = await fetch(`http://145.223.103.219:8080/presupuestos/desde-albaran`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload)
          });
          ...
          setMensaje(`Presupuesto ${presupuesto.numero} creado correctamente`);
          cerrarModalTransformar();
          break;
        }
  ...
  // ========== ADJUNTOS ==========
  const subirAdjunto = useCallback(async (archivo) => {
    if (!archivo) {
      setMensaje("Selecciona un archivo");
      return null;
    }
    
    const numeroAlbaran = formAlbaran.numero || "SIN-NUMERO";
    const rutaCarpeta = "/ventas/albaranes/adjuntos";
    const nombreArchivo = `${numeroAlbaran}_${archivo.name}`;
    
    // Crear carpetas si no existen
    const carpetas = ["/ventas", "/ventas/albaranes", "/ventas/albaranes/adjuntos"];
    for (const carpeta of carpetas) {
      try {
        ...
        await fetch(`${ARCHIVOS_API_URL}/carpeta`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ rutaCarpeta: rutaPadre, nombre: nombreCarpeta })
        });
      } catch (err) {
        // Ignorar errores si la carpeta ya existe
      }
    }
    ...
    formData.append("rutaCarpeta", rutaCarpeta);

    try {
      const res = await fetch(`${ARCHIVOS_API_URL}/subir`, {
        method: "POST",
        body: formData
      });
      ...
      if (res.ok) {
        const data = await res.json();
        setMensaje("Archivo adjuntado correctamente");
        // Añadir a la lista de adjuntos del formulario
        setFormAlbaran(prev => ({
          ...prev,
  ...
  }, [formAlbaran.numero, setMensaje]);

  const eliminarAdjunto = useCallback((index) => {
    setFormAlbaran(prev => ({
      ...prev,
  ...
  const descargarAdjunto = useCallback(async (adjunto) => {
    if (!adjunto?.id) {
      setMensaje("No se puede descargar el archivo");
      return;
    }
    try {
      const res = await fetch(`${ARCHIVOS_API_URL}/descargar/${adjunto.id}`);
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = adjunto.nombreOriginal || adjunto.nombre;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const abrirNuevoAlbaran = useCallback(async () => {
    const seriesActualizadas = await cargarSeries();
  ...
  }, [abrirPestana, generarNumeroParaSerie, cargarSeries, usuarioId]);
  ...
  const abrirEditarAlbaran = useCallback(
    async (albaran) => {
      ...
      setEstadoOriginalAlbaran(detalle.estado || estadoOptions[0]?.nombre || "Pendiente");
  ...
  const guardarAlbaran = useCallback(async (e, opciones = {}) => {
    ...
    const estadoActual = estadoOriginalAlbaran || formAlbaran.estado;
    ...
    if (eraEmitido && !esEmitido && documentoDescuentaStock === "ALBARAN" && !confirmarCambioEstado) {
    ...
    if (!eraEmitido && esEmitido && documentoDescuentaStock === "ALBARAN" && !confirmarStock) {
  ...
  const cancelarCambioEstado = useCallback(() => {
    ...
    if (estadoOriginalAlbaran) {
      setFormAlbaran(prev => ({
        ...prev,
        estado: estadoOriginalAlbaran
      }), pestanaActiva);
    }
