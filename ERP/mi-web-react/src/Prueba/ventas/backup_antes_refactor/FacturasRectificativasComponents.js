...
import { useFacturasRectificativasForm } from './useFacturasRectificativasForm';
...
const ESTADOS_FACTURA_RECTIFICATIVA = [
...
const COLUMNAS_FACTURA_RECTIFICATIVA = [
  ...
  { accessor: 'facturaOrigen.numero', header: 'Factura Origen', tipo: 'texto' },
...
export const FacturasRectificativasListado = ({ onEditar, onNuevo, modoVisual = "claro", abrirModalHistorialDocumento: abrirModalHistorialProp }) => {
  const {
    documentos,
    loading,
    paginacion,
  ...
  const eliminarSeleccionados = async () => {
    ...
    if (window.confirm(`¿Está seguro de eliminar ${documentosSeleccionados.length} factura(s) rectificativa(s)?`)) {
      try {
  ...
  useEffect(() => {
    ...
    const cargarSeriesPorTipo = async () => {
      const tipoDocumentoMap = {
        'DUPLICAR': 'FACTURA_RECTIFICATIVA',
  ...
  const ejecutarTransformacion = async () => {
    ...
    try {
      switch (tipoTransformacionSeleccionado) {
        case 'DUPLICAR': {
          const payload = {
            facturaRectificativaId: facturaParaTransformar.id,
          ...
          const res = await fetch('http://145.223.103.219:8080/facturas-rectificativas/duplicar', {
            method: 'POST',
          ...
          alert(`Factura Rectificativa ${creado.numero} duplicada correctamente`);
        ...
        case 'ALBARAN': {
          ...
          const res = await fetch('http://145.223.103.219:8080/albaranes/desde-factura-rectificativa', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'FACTURA': {
          ...
          const res = await fetch('http://145.223.103.219:8080/facturas/desde-factura-rectificativa', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'PEDIDO': {
          ...
          const res = await fetch('http://145.223.103.219:8080/pedidos/desde-factura-rectificativa', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'PRESUPUESTO': {
          ...
          const res = await fetch('http://145.223.103.219:8080/presupuestos/desde-factura-rectificativa', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'FACTURA_PROFORMA': {
          ...
          const res = await fetch('http://145.223.103.219:8080/facturas-proforma/desde-factura-rectificativa', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
  ...
  return (
    <DocumentoVentaListado
      tituloSingular="factura rectificativa"
      documentos={documentos}
      cargando={loading}
      estadoOptions={ESTADOS_FACTURA_RECTIFICATIVA}
    ...
    >
      {modalTransformarAbierto && facturaParaTransformar && (
        <ModalTransformarFacturaRectificativa
          modalTransformarAbierto={modalTransformarAbierto}
...
export function ModalTransformarFacturaRectificativa({
  ...
  const opcionesTransformacion = [
    { tipo: 'DUPLICAR', label: 'Duplicar Factura Rectificativa', icon: IconDuplicate, className: 'erp-btn-secondary' },
    ...
    <div className="erp-modal-overlay" onClick={cerrarModalTransformar}>
      <div className="erp-modal" onClick={(e) => e.stopPropagation()}>
        <div className="erp-modal-header">
          <h3>Transformar Factura Rectificativa {facturaParaTransformar.numero}</h3>
          ...
          {!tipoTransformacionSeleccionado ? (
            <>
              <p style={{ marginBottom: '20px', color: '#6b7280' }}>
                Selecciona qué acción deseas realizar con esta factura rectificativa:
              </p>
...
export default FacturasRectificativasListado;
