...
const ESTADOS_PRESUPUESTO = [
  ...
  { nombre: "Caducado", colorClaro: "#E5E7EB55", colorOscuro: "#37415155" },
];
...
export const PresupuestosListado = ({ onEditar, onNuevo, modoVisual = "claro", abrirModalHistorialDocumento: abrirModalHistorialProp }) => {
  const {
    documentos,
    loading,
    paginacion,
  ...
  const eliminarSeleccionados = async () => {
    ...
    if (window.confirm(`¿Está seguro de eliminar ${documentosSeleccionados.length} presupuesto(s)?`)) {
      try {
  ...
  const ejecutarTransformacion = async () => {
    if (!presupuestoParaTransformar || !tipoTransformacionSeleccionado) return;

    try {
      switch (tipoTransformacionSeleccionado) {
        case 'DUPLICAR': {
          const payload = {
            presupuestoId: presupuestoParaTransformar.id,
            serieId: serieSeleccionada ? parseInt(serieSeleccionada) : null,
            fecha: fechaTransformacion,
            estado: estadoTransformacion,
            descuentoAgrupacion: presupuestoParaTransformar.descuentoAgrupacion || 0
          };

          const res = await fetch('http://145.223.103.219:8080/presupuestos/duplicar', {
            method: 'POST',
          ...
          alert(`Presupuesto ${creado.numero} duplicado correctamente`);
        ...
        case 'ALBARAN': {
          ...
          const res = await fetch('http://145.223.103.219:8080/albaranes/desde-presupuesto', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'FACTURA': {
          ...
          const res = await fetch('http://145.223.103.219:8080/facturas/desde-presupuesto', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'PEDIDO': {
          ...
          const res = await fetch('http://145.223.103.219:8080/pedidos/desde-presupuesto', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'FACTURA_PROFORMA': {
          ...
          const res = await fetch('http://145.223.103.219:8080/facturas-proforma/desde-presupuesto', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'FACTURA_RECTIFICATIVA': {
          ...
          const res = await fetch('http://145.223.103.219:8080/facturas-rectificativas/desde-presupuesto', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
  ...
  return (
    <DocumentoVentaListado
      ...
      cargando={loading}
      estadoOptions={ESTADOS_PRESUPUESTO}
      ...
      documentoParaTransformar={presupuestoParaTransformar}
    ...
    >
      {modalTransformarAbierto && presupuestoParaTransformar && (
        <ModalTransformarPresupuesto
          modalTransformarAbierto={modalTransformarAbierto}
          presupuestoParaTransformar={presupuestoParaTransformar}
          cerrarModalTransformar={cerrarModalTransformar}
          tipoTransformacionSeleccionado={tipoTransformacionSeleccionado}
          setTipoTransformacionSeleccionado={setTipoTransformacionSeleccionado}
          serieSeleccionada={serieSeleccionada}
      ...
      {modalHistorialAbierto && (
        <div className="erp-modal-overlay" onClick={cerrarModalHistorial}>
          <div className="erp-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '800px' }}>
            <div className="erp-modal-header">
              <h3>Historial de Transformaciones</h3>
              <button className="erp-modal-close" onClick={cerrarModalHistorial}>×</button>
            </div>
            <div className="erp-modal-body">
              <h4 style={{ marginBottom: '16px' }}>
                Presupuesto {documentoHistorial?.numero}
              </h4>
              {cargandoHistorialModal ? (
                ...
                <table className="erp-table" style={{ width: '100%' }}>
                  ...
                  <tbody>
                    {historialModal.map((item, index) => (
                      <tr key={index}>
                        <td>
                          <span className={`erp-badge ${item.tipoTransformacion === 'DUPLICAR' ? 'erp-badge-secondary' : 'erp-badge-info'}`}>
                            {item.tipoTransformacion === 'DUPLICAR' ? 'Duplicado' : 'Convertido'}
                          </span>
                        </td>
                        <td>{item.numeroOrigen} ({item.tipoOrigen})</td>
                        <td>{item.numeroDestino} ({item.tipoDestino})</td>
                        <td>{new Date(item.fechaTransformacion).toLocaleString('es-ES')}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
export function ModalTransformarPresupuesto({
  modalTransformarAbierto,
  presupuestoParaTransformar,
  cerrarModalTransformar,
  tipoTransformacionSeleccionado,
  setTipoTransformacionSeleccionado,
  serieSeleccionada,
  ...
  if (!modalTransformarAbierto || !presupuestoParaTransformar) return null;

  const opcionesTransformacion = [
    { tipo: 'DUPLICAR', label: 'Duplicar Presupuesto', icon: IconDuplicate, className: 'erp-btn-secondary' },
  ...
  return (
    <div className="erp-modal-overlay" onClick={cerrarModalTransformar}>
      <div className="erp-modal" onClick={(e) => e.stopPropagation()}>
        <div className="erp-modal-header">
          <h3>Transformar Presupuesto {presupuestoParaTransformar.numero}</h3>
          ...
          {!tipoTransformacionSeleccionado ? (
            <>
              <p style={{ marginBottom: '20px', color: '#6b7280' }}>
                Selecciona qué acción deseas realizar con este presupuesto:
              </p>
...
export default PresupuestosListado;
