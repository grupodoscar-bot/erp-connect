...
const ESTADOS_PEDIDO = [
  ...
  { nombre: "En Proceso", colorClaro: "#C7D2FE55", colorOscuro: "#312E8155" },
  { nombre: "Completado", colorClaro: "#A7F3D055", colorOscuro: "#065F4655" },
...
export const PedidosListado = ({ onEditar, onNuevo, modoVisual = "claro", abrirModalHistorialDocumento: abrirModalHistorialProp }) => {
  const {
    documentos,
    loading,
    paginacion,
  ...
  const eliminarSeleccionados = async () => {
    ...
    if (window.confirm(`¿Está seguro de eliminar ${documentosSeleccionados.length} pedido(s)?`)) {
      try {
  ...
  const ejecutarTransformacion = async () => {
    if (!pedidoParaTransformar || !tipoTransformacionSeleccionado) return;

    try {
      switch (tipoTransformacionSeleccionado) {
        case 'DUPLICAR': {
          const payload = {
            pedidoId: pedidoParaTransformar.id,
            serieId: serieSeleccionada ? parseInt(serieSeleccionada) : null,
            fecha: fechaTransformacion,
            estado: estadoTransformacion,
            descuentoAgrupacion: pedidoParaTransformar.descuentoAgrupacion || 0
          };

          const res = await fetch('http://145.223.103.219:8080/pedidos/duplicar', {
            method: 'POST',
          ...
          alert(`Pedido ${creado.numero} duplicado correctamente`);
        ...
        case 'ALBARAN': {
          ...
          const res = await fetch('http://145.223.103.219:8080/albaranes/desde-pedido', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'FACTURA': {
          ...
          const res = await fetch('http://145.223.103.219:8080/facturas/desde-pedido', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'PRESUPUESTO': {
          ...
          const res = await fetch('http://145.223.103.219:8080/presupuestos/desde-pedido', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'FACTURA_PROFORMA': {
          ...
          const res = await fetch('http://145.223.103.219:8080/facturas-proforma/desde-pedido', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
        ...
        case 'FACTURA_RECTIFICATIVA': {
          ...
          const res = await fetch('http://145.223.103.219:8080/facturas-rectificativas/desde-pedido', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
  ...
  return (
    <DocumentoVentaListado
      tituloSingular="pedido"
      documentos={documentos}
      cargando={loading}
      estadoOptions={ESTADOS_PEDIDO}
      ...
      documentoParaTransformar={pedidoParaTransformar}
    ...
    >
      {modalTransformarAbierto && pedidoParaTransformar && (
        <ModalTransformarPedido
          modalTransformarAbierto={modalTransformarAbierto}
          pedidoParaTransformar={pedidoParaTransformar}
          cerrarModalTransformar={cerrarModalTransformar}
          tipoTransformacionSeleccionado={tipoTransformacionSeleccionado}
          setTipoTransformacionSeleccionado={setTipoTransformacionSeleccionado}
          serieSeleccionada={serieSeleccionada}
...
export function ModalTransformarPedido({
  modalTransformarAbierto,
  pedidoParaTransformar,
  cerrarModalTransformar,
  tipoTransformacionSeleccionado,
  setTipoTransformacionSeleccionado,
  serieSeleccionada,
  ...
  if (!modalTransformarAbierto || !pedidoParaTransformar) return null;

  const opcionesTransformacion = [
    { tipo: 'DUPLICAR', label: 'Duplicar Pedido', icon: IconDuplicate, className: 'erp-btn-secondary' },
    ...
    <div className="erp-modal-overlay" onClick={cerrarModalTransformar}>
      <div className="erp-modal" onClick={(e) => e.stopPropagation()}>
        <div className="erp-modal-header">
          <h3>Transformar Pedido {pedidoParaTransformar.numero}</h3>
          ...
          {!tipoTransformacionSeleccionado ? (
            <>
              <p style={{ marginBottom: '20px', color: '#6b7280' }}>
                Selecciona qué acción deseas realizar con este pedido:
              </p>
