            ...
            <tbody>
              {albaranes.map((a) => (
                ...
                >
                  ...
                  <td className="erp-td-mono">{a.fecha}</td>
...
export function FormularioAlbaran({ 
  ...
  const totales = {
    ...
    baseTrasAgrupacion: Number.parseFloat(totalesBase?.baseTrasAgrupacion) || 0,
  ...
  // Actualizar direcciones cuando cambia el cliente
  useEffect(() => {
    if (clienteSeleccionado) {
      // Llamar al backend para obtener las direcciones del cliente
      const obtenerDirecciones = async () => {
        try {
          // Hacer llamada al backend (Spring Boot)
          const response = await fetch(`http://145.223.103.219:8080/clientes/${clienteSeleccionado.id}/direcciones`);
      ...
      for (let i = 0; i < formAlbaran.lineas.length; i++) {
        ...
        try {
          const response = await fetch(
            `http://145.223.103.219:8080/productos/${linea.productoId}/stock/${almacenId}`
          );
          if (response.ok) {
                ...
                <label className="erp-field">
                  ...
                  {!formAlbaran.usarCodigoManual && (
                    <small style={{ color: '#6b7280', display: 'block', marginTop: '4px' }}>
                      {generandoNumero ? "Generando número..." : "El número se asigna automáticamente según la serie"}
                    </small>
                  )}
                </label>
                <label className="erp-field">
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', gap: '16px', flexWrap: 'wrap' }}>
                ...
                {clienteSeleccionado && (
                  <span style={{ fontSize: '12px', color: '#6b7280' }}>
                    Se guardan como snapshot dentro del {tipoDocumento} (no modifican el cliente).
                  </span>
                )}
              </div>
...
export function FichaAlbaran({ 
  ...
  useEffect(() => {
    ...
    const obtenerDirecciones = async () => {
      try {
        const response = await fetch(`http://145.223.103.219:8080/clientes/${cliente.id}/direcciones`);
        if (response.ok) {
          const direcciones = await response.json();
          const direccionesFormateadas = direcciones.map(dir => ({
            id: dir.id,
        ...
        <div className="erp-form-section">
          <div className="erp-form-group">
            ...
            <div className="erp-form-row erp-form-row-4">
              ...
              <label className="erp-field">
                <span className="erp-field-label">Fecha</span>
                <input type="date" className="erp-input-mono" value={albaran.fecha || ''} disabled />
              </label>
              <label className="erp-field">
                <span className="erp-field-label">Cliente</span>
          ...
          <div className="erp-form-group">
            ...
            <div style={{ display: 'flex', gap: '24px', marginTop: '16px', justifyContent: 'flex-end' }}>
              ...
              {/* Totales a la derecha */}
              <div className="erp-totales-box" style={{ flex: '0 0 auto', minWidth: '320px' }}>
                <div className="erp-total-row">
                  <span>Subtotal:</span>
