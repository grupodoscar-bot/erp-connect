...
const API_URL_PEDIDOS_COMPRA = API_ENDPOINTS.pedidosCompra;

export function useTarifasPedidoCompra() {
  ...
  // Cargar tarifas disponibles para pedidos de compra
  const cargarTarifasDisponibles = useCallback(async () => {
    try {
      setCargandoTarifas(true);
      const res = await fetch(`${API_URL_PEDIDOS_COMPRA}/tarifas-disponibles`);
  ...
  const obtenerPrecioProducto = useCallback(async (productoId) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL_PEDIDOS_COMPRA}/precio-producto?${params}`);
  ...
  const cambiarTarifa = useCallback((tarifaId, tarifaObjetoCompleto = null) => {
    // Si se proporciona un objeto de tarifa completo, usarlo directamente
    if (tarifaObjetoCompleto && tarifaObjetoCompleto.id) {
      setTarifaSeleccionada(tarifaObjetoCompleto);
      return;
    }
    // Si no, buscar en las tarifas disponibles
  ...
  // Cambiar tarifa basada en el proveedor seleccionado
  const cambiarTarifaPorProveedor = useCallback((proveedor) => {
    if (!proveedor || !esMultitarifaPermitida) {
      resetearTarifaPorDefecto();
      return;
    }
    
    // Si el proveedor tiene tarifa asignada, usarla
    if (proveedor.tarifaAsignada?.id) {
      const tarifaProveedor = tarifasDisponibles.find(
        t => t.id === parseInt(proveedor.tarifaAsignada.id)
      );
      if (tarifaProveedor) {
        setTarifaSeleccionada(tarifaProveedor);
      } else {
        resetearTarifaPorDefecto();
      }
    } else {
