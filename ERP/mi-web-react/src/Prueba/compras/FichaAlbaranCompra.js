...
const normalizarAlbaran = (albaran) => {
  if (!albaran) return null;

  const compraMultialmacen = albaran.compraMultialmacen ?? albaran.ventaMultialmacen ?? false;

  return {
    ...albaran,
    compraMultialmacen,
    ventaMultialmacen: compraMultialmacen,
    lineas: Array.isArray(albaran.lineas) ? albaran.lineas : [],
    tarifa: albaran.tarifa || (albaran.tarifaId ? { id: albaran.tarifaId, nombre: `Tarifa ${albaran.tarifaId}` } : null),
    almacen: albaran.almacen || (albaran.almacenId ? { id: albaran.almacenId, nombre: `Almacén ${albaran.almacenId}` } : null),
    tarifaId: albaran.tarifaId ?? albaran.tarifa?.id ?? null,
    almacenId: albaran.almacenId ?? albaran.almacen?.id ?? null,
  };
};

const obtenerValor = (albaran, campos = []) => {
...
const obtenerNombreSerie = (albaran) => {
  return obtenerValor(albaran, [
    'serie.prefijo',
    'serie.nombre',
    'serieNombre'
  ]) || 'Sin serie';
...
const obtenerNombreTarifa = (albaran) => {
  return (
    obtenerValor(albaran, ['tarifa.nombre', 'tarifaNombre']) ||
    (albaran?.tarifaId ? `Tarifa ${albaran.tarifaId}` : '—')
  );
};

const obtenerNombreAlmacen = (albaran) => {
  return (
    obtenerValor(albaran, ['almacen.nombre', 'almacenNombre']) ||
    (albaran?.almacenId ? `Almacén ${albaran.almacenId}` : '—')
  );
};

// Componente de ficha (vista de solo lectura) para albarán de compra
export function FichaAlbaranCompra({ 
  albaranesCompra = [],
  albaranId, 
  abrirEditarAlbaran, 
  generarPdf,
  ...
  const albaranData = useMemo(() => normalizarAlbaran(albaran), [albaran]);
  const proveedor = albaranData?.proveedor;
  ...
  useEffect(() => {
    if (!albaranId) return;
    const albaranEnLista = albaranesCompra.find((a) => a.id === albaranId);
    if (albaranEnLista) {
      setAlbaran(normalizarAlbaran(albaranEnLista));
    }
  }, [albaranId, albaranesCompra]);

  // Cargar siempre el albarán completo desde backend
  useEffect(() => {
    if (!albaranId) return;
    let cancelado = false;
    ...
    const cargarAlbaran = async () => {
      ...
      try {
        const response = await fetch(`${API_ENDPOINTS.albaranesCompra}/${albaranId}`);
        if (response.ok) {
          const data = await response.json();
          if (!cancelado) {
            setAlbaran(normalizarAlbaran(data));
          }
        }
      } catch (error) {
        if (!cancelado) {
  ...
  useEffect(() => {
    if (!albaranData?.id || !cargarHistorialTransformaciones) return;

    const cargarHistorial = async () => {
      setCargandoHistorial(true);
      try {
        const historial = await cargarHistorialTransformaciones('ALBARAN_COMPRA', albaranData.id);
    ...
    cargarHistorial();
  }, [albaranData?.id, cargarHistorialTransformaciones]);

  if (cargandoAlbaran && !albaranData) {
    return <div className="erp-empty-state">Cargando albarán de compra...</div>;
  }
  if (!albaranData) return <div className="erp-empty-state">Albarán de compra no encontrado</div>;
  ...
  const calcularTotales = () => {
    ...
    (albaranData.lineas || []).forEach(linea => {
      ...
      const descuentoAgrupacionPct = albaranData.descuentoAgrupacion || 0;
  ...
  return (
    <div className="erp-form-view">
      ...
      <div className="erp-form-actions" style={{ marginBottom: '16px', paddingTop: 0 }}>
        <button type="button" className="erp-btn erp-btn-primary" onClick={() => abrirEditarAlbaran(albaranData)}>
          <IconEdit className="erp-action-icon" /> Editar
        </button>
        {generarPdf && (
          <button type="button" className="erp-btn erp-btn-info" onClick={() => generarPdf(albaranData.id)}>
            <IconPdf className="erp-action-icon" /> PDF
          </button>
        )}
        {abrirModalTransformar && (
          <button type="button" className="erp-btn erp-btn-secondary" onClick={() => abrirModalTransformar(albaranData)}>
            <IconTransform className="erp-action-icon" /> Transformar
          </button>
        )}
        {abrirModalEmail && (
          <button type="button" className="erp-btn erp-btn-secondary" onClick={() => abrirModalEmail(albaranData)}>
      ...
      <div className="erp-form-content">
        <div className="erp-form-section">
          <div className="erp-form-group">
            <h4 className="erp-form-group-title">Datos del albarán de compra</h4>
            <div className="erp-form-row erp-form-row-4">
              <label className="erp-field">
                <span className="erp-field-label">Número</span>
                <input type="text" className="erp-input-mono" value={albaranData.numero || ''} disabled />
              </label>
              <label className="erp-field">
                <span className="erp-field-label">Fecha</span>
                <input type="date" className="erp-input-mono" value={formatearSoloFecha(albaranData.fecha) || ''} disabled />
              </label>
              <label className="erp-field">
                <span className="erp-field-label">Proveedor</span>
                <input type="text" value={albaranData.proveedorNombreComercial || proveedor?.nombreComercial || 'Sin proveedor'} disabled />
              </label>
              <label className="erp-field">
                ...
                <input
                  type="text"
                  className="erp-input"
                  value={albaranData.estado || ''}
                  disabled
                  style={{
                    textTransform: 'uppercase',
                    color: modoVisual === "oscuro" ? "#0f172a" : "#1f2937",
                    fontWeight: 600,
                    ...getLineaEstadoStyle(albaranData.estado)
                  }}
                />
              </label>
            </div>
            ...
            <div className="erp-form-row erp-form-row-3" style={{ marginTop: '12px' }}>
              <label className="erp-field">
                ...
                <input type="text" value={obtenerNombreSerie(albaranData)} disabled />
              </label>

              <label className="erp-field">
                <span className="erp-field-label">Tarifa</span>
                <input type="text" value={obtenerNombreTarifa(albaranData)} disabled />
              </label>

              {!albaranData.ventaMultialmacen ? (
                <label className="erp-field">
                  <span className="erp-field-label">Almacén</span>
                  <input type="text" value={obtenerNombreAlmacen(albaranData)} disabled />
                </label>
              ) : (
                <label className="erp-field">
            ...
            {(albaranData.proveedorNombreComercial || proveedor) && (
              <div style={{ 
                marginTop: '16px', 
                padding: '12px 16px', 
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '8px 24px' }}>
                  <div><span style={{ color: '#64748b' }}>Nombre comercial:</span> <strong>{albaranData.proveedorNombreComercial || proveedor?.nombreComercial || '—'}</strong></div>
                  <div><span style={{ color: '#64748b' }}>Razón social:</span> {albaranData.proveedorNombreFiscal || proveedor?.nombreFiscal || '—'}</div>
                  <div><span style={{ color: '#64748b' }}>CIF/NIF:</span> <span style={{ fontFamily: 'monospace' }}>{albaranData.proveedorNifCif || proveedor?.nifCif || '—'}</span></div>
                  <div><span style={{ color: '#64748b' }}>Email:</span> {albaranData.proveedorEmail || proveedor?.email || '—'}</div>
                  <div><span style={{ color: '#64748b' }}>Teléfono:</span> {albaranData.proveedorTelefono || proveedor?.telefonoFijo || proveedor?.telefonoMovil || '—'}</div>
                  {agrupacionProveedor && (
            ...
            <div className="erp-form-row erp-form-row-2" style={{ marginTop: '16px', gap: '16px' }}>
              ...
              <div className="erp-field">
                ...
                <div style={{ 
                  ...
                  {(() => {
                    const partes = [];
                    if (albaranData.direccionFacturacionDireccion) partes.push(albaranData.direccionFacturacionDireccion);
                    if (albaranData.direccionFacturacionCodigoPostal) partes.push(albaranData.direccionFacturacionCodigoPostal);
                    if (albaranData.direccionFacturacionPoblacion) partes.push(albaranData.direccionFacturacionPoblacion);
                    if (albaranData.direccionFacturacionProvincia) partes.push(albaranData.direccionFacturacionProvincia);
                    if (albaranData.direccionFacturacionPais && albaranData.direccionFacturacionPais !== 'España') partes.push(albaranData.direccionFacturacionPais);
              ...
              <div className="erp-field">
                ...
                <div style={{ 
                  ...
                  {(() => {
                    const partes = [];
                    if (albaranData.direccionEnvioDireccion) partes.push(albaranData.direccionEnvioDireccion);
                    if (albaranData.direccionEnvioCodigoPostal) partes.push(albaranData.direccionEnvioCodigoPostal);
                    if (albaranData.direccionEnvioPoblacion) partes.push(albaranData.direccionEnvioPoblacion);
                    if (albaranData.direccionEnvioProvincia) partes.push(albaranData.direccionEnvioProvincia);
                    if (albaranData.direccionEnvioPais && albaranData.direccionEnvioPais !== 'España') partes.push(albaranData.direccionEnvioPais);
                    if (partes.length > 0) return partes.join(', ');
                    
                    if (albaranData.direccion) {
                      const dir = albaranData.direccion;
                      const partesFallback = [];
          ...
          <div className="erp-form-group">
            ...
            <table className="erp-table erp-table-compact">
              <thead>
                <tr>
                  <th style={{ width: albaranData.ventaMultialmacen ? '7%' : '8%' }}>Referencia</th>
                  <th style={{ width: albaranData.ventaMultialmacen ? '22%' : '30%' }}>Producto</th>
                  {albaranData.ventaMultialmacen && (
                    <th style={{ width: '13%' }}>Almacén</th>
                  )}
                  <th style={{ width: '7%', textAlign: 'center' }}>Cant.</th>
                  <th style={{ width: albaranData.ventaMultialmacen ? '11%' : '14%', textAlign: 'center' }}>Precio</th>
                  <th style={{ width: '7%', textAlign: 'center' }}>Dto. %</th>
                  <th style={{ width: '7%', textAlign: 'center' }}>IVA %</th>
                  <th style={{ width: '7%', textAlign: 'center' }}>Rec. %</th>
                  <th style={{ width: albaranData.ventaMultialmacen ? '11%' : '18%', textAlign: 'center' }}>Base</th>
                  <th style={{ width: '4%' }}></th>
                </tr>
              </thead>
              <tbody>
                {(albaranData.lineas || []).map((linea, index) => (
                  <tr key={index}>
                ...
                {(!albaranData.lineas || albaranData.lineas.length === 0) && (
                  <tr>
                    <td colSpan={albaranData.ventaMultialmacen ? 10 : 9} className="erp-td-empty">
          ...
          <div className="erp-form-group">
            <div className="erp-form-row erp-form-row-2">
              <label className="erp-field">
                ...
                <textarea
                  rows="3"
                  value={albaranData.observaciones || ''}
                  disabled
                  placeholder="—"
                />
              </label>
          ...
          {/* Adjuntos */}
          {albaranData.adjuntos && albaranData.adjuntos.length > 0 && (
            <div className="erp-form-group">
              <h4 className="erp-form-group-title">Adjuntos</h4>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {albaranData.adjuntos.map((adj, idx) => (
                  <button
                    key={idx}
                    type="button"
