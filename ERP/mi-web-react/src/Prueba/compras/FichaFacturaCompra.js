...
const normalizarFactura = (factura) => {
  ...
  const compraMultialmacen = factura.compraMultialmacen ?? factura.ventaMultialmacen ?? false;
  ...
  return {
    ...
    tarifa: factura.tarifa || (factura.tarifaId ? { id: factura.tarifaId, nombre: `Tarifa ${factura.tarifaId}` } : null),
    almacen: factura.almacen || (factura.almacenId ? { id: factura.almacenId, nombre: `Almacén ${factura.almacenId}` } : null),
    tarifaId: factura.tarifaId ?? factura.tarifa?.id ?? null,
    almacenId: factura.almacenId ?? factura.almacen?.id ?? null,
  };
};

const obtenerValor = (factura, campos = []) => {
...
const obtenerNombreSerie = (factura) => {
  return obtenerValor(factura, [
    'serie.prefijo',
    'serie.nombre',
    'serieNombre'
  ]) || 'Sin serie';
...
const obtenerNombreTarifa = (factura) => {
  return (
    obtenerValor(factura, ['tarifa.nombre', 'tarifaNombre']) ||
    (factura?.tarifaId ? `Tarifa ${factura.tarifaId}` : '—')
  );
};

const obtenerNombreAlmacen = (factura) => {
  return (
    obtenerValor(factura, ['almacen.nombre', 'almacenNombre']) ||
    (factura?.almacenId ? `Almacén ${factura.almacenId}` : '—')
  );
};

// Componente de ficha (vista de solo lectura) para factura de compra
export function FichaFacturaCompra({ 
  facturasCompra = [],
  facturaId, 
  abrirEditarFactura, 
  generarPdf,
  ...
  const facturaData = useMemo(() => normalizarFactura(factura), [factura]);
  const proveedor = facturaData?.proveedor;
  ...
  useEffect(() => {
    if (!facturaId) return;
    const facturaEnLista = facturasCompra.find((f) => f.id === facturaId);
    if (facturaEnLista) {
      setFactura(normalizarFactura(facturaEnLista));
    }
  }, [facturaId, facturasCompra]);

  // Cargar siempre la factura completa desde backend
  useEffect(() => {
    if (!facturaId) return;
    let cancelado = false;
    ...
    const cargarFactura = async () => {
      ...
      try {
        const response = await fetch(`${API_ENDPOINTS.facturasCompra}/${facturaId}`);
        if (response.ok) {
          const data = await response.json();
          if (!cancelado) {
            setFactura(normalizarFactura(data));
          }
        }
      } catch (error) {
        if (!cancelado) {
  ...
  useEffect(() => {
    if (!facturaData?.id || !cargarHistorialTransformaciones) return;

    const cargarHistorial = async () => {
      setCargandoHistorial(true);
      try {
        const historial = await cargarHistorialTransformaciones('FACTURA_COMPRA', facturaData.id);
    ...
    cargarHistorial();
  }, [facturaData?.id, cargarHistorialTransformaciones]);

  if (cargandoFactura && !facturaData) {
    return <div className="erp-empty-state">Cargando factura de compra...</div>;
  }
  if (!facturaData) return <div className="erp-empty-state">Factura de compra no encontrada</div>;
  ...
  const calcularTotales = () => {
    ...
    (facturaData.lineas || []).forEach(linea => {
      ...
      const descuentoAgrupacionPct = facturaData.descuentoAgrupacion || 0;
  ...
  return (
    <div className="erp-form-view">
      ...
      <div className="erp-form-actions" style={{ marginBottom: '16px', paddingTop: 0 }}>
        <button type="button" className="erp-btn erp-btn-primary" onClick={() => abrirEditarFactura(facturaData)}>
          <IconEdit className="erp-action-icon" /> Editar
        </button>
        {generarPdf && (
          <button type="button" className="erp-btn erp-btn-info" onClick={() => generarPdf(facturaData.id)}>
            <IconPdf className="erp-action-icon" /> PDF
          </button>
        )}
        {abrirModalTransformar && (
          <button type="button" className="erp-btn erp-btn-secondary" onClick={() => abrirModalTransformar(facturaData)}>
            <IconTransform className="erp-action-icon" /> Transformar
          </button>
        )}
        {abrirModalEmail && (
          <button type="button" className="erp-btn erp-btn-secondary" onClick={() => abrirModalEmail(facturaData)}>
      ...
      <div className="erp-form-content">
        <div className="erp-form-section">
          <div className="erp-form-group">
            <h4 className="erp-form-group-title">Datos de la factura de compra</h4>
            <div className="erp-form-row erp-form-row-4">
              <label className="erp-field">
                <span className="erp-field-label">Número</span>
                <input type="text" className="erp-input-mono" value={facturaData.numero || ''} disabled />
              </label>
              <label className="erp-field">
                <span className="erp-field-label">Fecha</span>
                <input type="date" className="erp-input-mono" value={formatearSoloFecha(facturaData.fecha) || ''} disabled />
              </label>
              <label className="erp-field">
                <span className="erp-field-label">Proveedor</span>
                <input type="text" value={facturaData.proveedorNombreComercial || proveedor?.nombreComercial || 'Sin proveedor'} disabled />
              </label>
              <label className="erp-field">
                ...
                <input
                  type="text"
                  className="erp-input"
                  value={facturaData.estado || ''}
                  disabled
                  style={{
                    textTransform: 'uppercase',
                    color: modoVisual === "oscuro" ? "#0f172a" : "#1f2937",
                    fontWeight: 600,
                    ...getLineaEstadoStyle(facturaData.estado)
                  }}
                />
              </label>
            </div>
            ...
            <div className="erp-form-row erp-form-row-3" style={{ marginTop: '12px' }}>
              <label className="erp-field">
                ...
                <input type="text" value={obtenerNombreSerie(facturaData)} disabled />
              </label>

              <label className="erp-field">
                <span className="erp-field-label">Tarifa</span>
                <input type="text" value={obtenerNombreTarifa(facturaData)} disabled />
              </label>

              {!facturaData.ventaMultialmacen ? (
                <label className="erp-field">
                  <span className="erp-field-label">Almacén</span>
                  <input type="text" value={obtenerNombreAlmacen(facturaData)} disabled />
                </label>
              ) : (
                <label className="erp-field">
            ...
            {(facturaData.proveedorNombreComercial || proveedor) && (
              <div style={{ 
                marginTop: '16px', 
                padding: '12px 16px', 
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '8px 24px' }}>
                  <div><span style={{ color: '#64748b' }}>Nombre comercial:</span> <strong>{facturaData.proveedorNombreComercial || proveedor?.nombreComercial || '—'}</strong></div>
                  <div><span style={{ color: '#64748b' }}>Razón social:</span> {facturaData.proveedorNombreFiscal || proveedor?.nombreFiscal || '—'}</div>
                  <div><span style={{ color: '#64748b' }}>CIF/NIF:</span> <span style={{ fontFamily: 'monospace' }}>{facturaData.proveedorNifCif || proveedor?.nifCif || '—'}</span></div>
                  <div><span style={{ color: '#64748b' }}>Email:</span> {facturaData.proveedorEmail || proveedor?.email || '—'}</div>
                  <div><span style={{ color: '#64748b' }}>Teléfono:</span> {facturaData.proveedorTelefono || proveedor?.telefonoFijo || proveedor?.telefonoMovil || '—'}</div>
                  {agrupacionProveedor && (
            ...
            <div className="erp-form-row erp-form-row-2" style={{ marginTop: '16px', gap: '16px' }}>
              ...
              <div className="erp-field">
                ...
                <div style={{ 
                  ...
                  {(() => {
                    const partes = [];
                    if (facturaData.direccionFacturacionDireccion) partes.push(facturaData.direccionFacturacionDireccion);
                    if (facturaData.direccionFacturacionCodigoPostal) partes.push(facturaData.direccionFacturacionCodigoPostal);
                    if (facturaData.direccionFacturacionPoblacion) partes.push(facturaData.direccionFacturacionPoblacion);
                    if (facturaData.direccionFacturacionProvincia) partes.push(facturaData.direccionFacturacionProvincia);
                    if (facturaData.direccionFacturacionPais && facturaData.direccionFacturacionPais !== 'España') partes.push(facturaData.direccionFacturacionPais);
              ...
              <div className="erp-field">
                ...
                <div style={{ 
                  ...
                  {(() => {
                    const partes = [];
                    if (facturaData.direccionEnvioDireccion) partes.push(facturaData.direccionEnvioDireccion);
                    if (facturaData.direccionEnvioCodigoPostal) partes.push(facturaData.direccionEnvioCodigoPostal);
                    if (facturaData.direccionEnvioPoblacion) partes.push(facturaData.direccionEnvioPoblacion);
                    if (facturaData.direccionEnvioProvincia) partes.push(facturaData.direccionEnvioProvincia);
                    if (facturaData.direccionEnvioPais && facturaData.direccionEnvioPais !== 'España') partes.push(facturaData.direccionEnvioPais);
                    if (partes.length > 0) return partes.join(', ');
                    
                    if (facturaData.direccion) {
                      const dir = facturaData.direccion;
                      const partesFallback = [];
          ...
          <div className="erp-form-group">
            ...
            <table className="erp-table erp-table-compact">
              <thead>
                <tr>
                  <th style={{ width: facturaData.ventaMultialmacen ? '7%' : '8%' }}>Referencia</th>
                  <th style={{ width: facturaData.ventaMultialmacen ? '22%' : '30%' }}>Producto</th>
                  {facturaData.ventaMultialmacen && (
                    <th style={{ width: '13%' }}>Almacén</th>
                  )}
                  <th style={{ width: '7%', textAlign: 'center' }}>Cant.</th>
                  <th style={{ width: facturaData.ventaMultialmacen ? '11%' : '14%', textAlign: 'center' }}>Precio</th>
                  <th style={{ width: '7%', textAlign: 'center' }}>Dto. %</th>
                  <th style={{ width: '7%', textAlign: 'center' }}>IVA %</th>
                  <th style={{ width: '7%', textAlign: 'center' }}>Rec. %</th>
                  <th style={{ width: facturaData.ventaMultialmacen ? '11%' : '18%', textAlign: 'center' }}>Base</th>
                  <th style={{ width: '4%' }}></th>
                </tr>
              </thead>
              <tbody>
                {(facturaData.lineas || []).map((linea, index) => (
                  <tr key={index}>
                ...
                {(!facturaData.lineas || facturaData.lineas.length === 0) && (
                  <tr>
                    <td colSpan={facturaData.ventaMultialmacen ? 10 : 9} className="erp-td-empty">
          ...
          {/* Adjuntos */}
          {facturaData.adjuntos && facturaData.adjuntos.length > 0 && (
            <div className="erp-form-group">
              <h4 className="erp-form-group-title">Adjuntos</h4>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {facturaData.adjuntos.map((adj, idx) => (
                  <button
                    key={idx}
                    type="button"
