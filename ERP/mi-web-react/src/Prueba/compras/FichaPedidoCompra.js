...
const normalizarPedido = (pedido) => {
  ...
  const compraMultialmacen = pedido.compraMultialmacen ?? pedido.ventaMultialmacen ?? false;

  return {
    ...pedido,
    compraMultialmacen,
    ventaMultialmacen: compraMultialmacen,
    lineas: Array.isArray(pedido.lineas) ? pedido.lineas : [],
    // Asegurar que tarifa y almacen estén disponibles como objetos o crear objetos planos
    tarifa: pedido.tarifa || (pedido.tarifaId ? { id: pedido.tarifaId, nombre: `Tarifa ${pedido.tarifaId}` } : null),
    almacen: pedido.almacen || (pedido.almacenId ? { id: pedido.almacenId, nombre: `Almacén ${pedido.almacenId}` } : null),
    tarifaId: pedido.tarifaId ?? pedido.tarifa?.id ?? null,
    almacenId: pedido.almacenId ?? pedido.almacen?.id ?? null,
  };
};

const obtenerValor = (pedido, campos = []) => {
...
const obtenerNombreSerie = (pedido) => {
  return obtenerValor(pedido, [
    'serie.prefijo',
    'serie.nombre',
    'serieNombre'
  ]) || 'Sin serie';
...
const obtenerNombreTarifa = (pedido) => {
  return (
    obtenerValor(pedido, ['tarifa.nombre', 'tarifaNombre']) ||
    (pedido?.tarifaId ? `Tarifa ${pedido.tarifaId}` : '—')
  );
};

const obtenerNombreAlmacen = (pedido) => {
  return (
    obtenerValor(pedido, ['almacen.nombre', 'almacenNombre']) ||
    (pedido?.almacenId ? `Almacén ${pedido.almacenId}` : '—')
  );
};

// Componente de ficha (vista de solo lectura) para pedido de compra
// Reutiliza la estructura de FichaAlbaran pero adaptado para compras (proveedor en lugar de cliente)
export function FichaPedidoCompra({ 
  pedidosCompra = [],
  pedidoId, 
  abrirEditarPedido, 
  generarPdf,
  ...
  const pedidoData = useMemo(() => normalizarPedido(pedido), [pedido]);
  const proveedor = pedidoData?.proveedor;
  ...
  useEffect(() => {
    if (!pedidoId) return;
    const pedidoEnLista = pedidosCompra.find((p) => p.id === pedidoId);
    if (pedidoEnLista) {
      setPedido(normalizarPedido(pedidoEnLista));
    }
  }, [pedidoId, pedidosCompra]);

  // Cargar siempre el pedido completo desde backend
  useEffect(() => {
    if (!pedidoId) return;
    let cancelado = false;
    ...
    const cargarPedido = async () => {
      ...
      try {
        const response = await fetch(`${API_ENDPOINTS.pedidosCompra}/${pedidoId}`);
        if (response.ok) {
          const data = await response.json();
          if (!cancelado) {
            setPedido(normalizarPedido(data));
          }
        }
      } catch (error) {
        if (!cancelado) {
  ...
  useEffect(() => {
    if (!pedidoData?.id || !cargarHistorialTransformaciones) return;

    const cargarHistorial = async () => {
      setCargandoHistorial(true);
      try {
        const historial = await cargarHistorialTransformaciones('PEDIDO_COMPRA', pedidoData.id);
    ...
    cargarHistorial();
  }, [pedidoData?.id, cargarHistorialTransformaciones]);

  if (cargandoPedido && !pedidoData) {
    return <div className="erp-empty-state">Cargando pedido de compra...</div>;
  }
  if (!pedidoData) return <div className="erp-empty-state">Pedido de compra no encontrado</div>;
  ...
  const calcularTotales = () => {
    ...
    (pedidoData.lineas || []).forEach(linea => {
      ...
      const descuentoAgrupacionPct = pedidoData.descuentoAgrupacion || 0;
    ...
    const descuentoAgrupacionPct = pedido.descuentoAgrupacion || 0;
  ...
  return (
    <div className="erp-form-view">
      ...
      <div className="erp-form-actions" style={{ marginBottom: '16px', paddingTop: 0 }}>
        <button type="button" className="erp-btn erp-btn-primary" onClick={() => abrirEditarPedido(pedidoData)}>
          <IconEdit className="erp-action-icon" /> Editar
        </button>
        {generarPdf && (
          <button type="button" className="erp-btn erp-btn-info" onClick={() => generarPdf(pedidoData.id)}>
            <IconPdf className="erp-action-icon" /> PDF
          </button>
        )}
        {abrirModalTransformar && (
          <button type="button" className="erp-btn erp-btn-secondary" onClick={() => abrirModalTransformar(pedidoData)}>
            <IconTransform className="erp-action-icon" /> Transformar
          </button>
        )}
        {abrirModalEmail && (
          <button type="button" className="erp-btn erp-btn-secondary" onClick={() => abrirModalEmail(pedidoData)}>
      ...
      <div className="erp-form-content">
        <div className="erp-form-section">
          <div className="erp-form-group">
            <h4 className="erp-form-group-title">Datos del pedido de compra</h4>
            <div className="erp-form-row erp-form-row-4">
              <label className="erp-field">
                <span className="erp-field-label">Número</span>
                <input type="text" className="erp-input-mono" value={pedidoData.numero || ''} disabled />
              </label>
              <label className="erp-field">
                <span className="erp-field-label">Fecha</span>
                <input type="date" className="erp-input-mono" value={formatearSoloFecha(pedidoData.fecha) || ''} disabled />
              </label>
              <label className="erp-field">
                <span className="erp-field-label">Proveedor</span>
                <input type="text" value={pedidoData.proveedorNombreComercial || proveedor?.nombreComercial || 'Sin proveedor'} disabled />
              </label>
              <label className="erp-field">
                ...
                <input
                  ...
                  style={{
                    ...
                    fontWeight: 600,
                    ...getLineaEstadoStyle(pedidoData.estado)
                  }}
                />
              </label>
            </div>
            ...
            <div className="erp-form-row erp-form-row-3" style={{ marginTop: '12px' }}>
              <label className="erp-field">
                ...
                <input type="text" value={obtenerNombreSerie(pedidoData)} disabled />
              </label>

              <label className="erp-field">
                <span className="erp-field-label">Tarifa</span>
                <input type="text" value={obtenerNombreTarifa(pedidoData)} disabled />
              </label>

              {!pedidoData.ventaMultialmacen ? (
                <label className="erp-field">
                  <span className="erp-field-label">Almacén</span>
                  <input type="text" value={obtenerNombreAlmacen(pedidoData)} disabled />
                </label>
              ) : (
                <label className="erp-field">
            ...
            {(pedidoData.proveedorNombreComercial || proveedor) && (
              <div style={{ 
                marginTop: '16px', 
                padding: '12px 16px', 
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '8px 24px' }}>
                  <div><span style={{ color: '#64748b' }}>Nombre comercial:</span> <strong>{pedidoData.proveedorNombreComercial || proveedor?.nombreComercial || '—'}</strong></div>
                  <div><span style={{ color: '#64748b' }}>Razón social:</span> {pedidoData.proveedorNombreFiscal || proveedor?.nombreFiscal || '—'}</div>
                  <div><span style={{ color: '#64748b' }}>CIF/NIF:</span> <span style={{ fontFamily: 'monospace' }}>{pedidoData.proveedorNifCif || proveedor?.nifCif || '—'}</span></div>
                  <div><span style={{ color: '#64748b' }}>Email:</span> {pedidoData.proveedorEmail || proveedor?.email || '—'}</div>
                  <div><span style={{ color: '#64748b' }}>Teléfono:</span> {pedidoData.proveedorTelefono || proveedor?.telefonoFijo || proveedor?.telefonoMovil || '—'}</div>
                  {agrupacionProveedor && (
            ...
            <div className="erp-form-row erp-form-row-2" style={{ marginTop: '16px', gap: '16px' }}>
              ...
              <div className="erp-field">
                ...
                <div style={{ 
                  ...
                  {(() => {
                    const partes = [];
                    if (pedidoData.direccionFacturacionDireccion) partes.push(pedidoData.direccionFacturacionDireccion);
                    if (pedidoData.direccionFacturacionCodigoPostal) partes.push(pedidoData.direccionFacturacionCodigoPostal);
                    if (pedidoData.direccionFacturacionPoblacion) partes.push(pedidoData.direccionFacturacionPoblacion);
                    if (pedidoData.direccionFacturacionProvincia) partes.push(pedidoData.direccionFacturacionProvincia);
                    if (pedidoData.direccionFacturacionPais && pedidoData.direccionFacturacionPais !== 'España') partes.push(pedidoData.direccionFacturacionPais);
              ...
              <div className="erp-field">
                ...
                <div style={{ 
                  ...
                  {(() => {
                    const partes = [];
                    if (pedidoData.direccionEnvioDireccion) partes.push(pedidoData.direccionEnvioDireccion);
                    if (pedidoData.direccionEnvioCodigoPostal) partes.push(pedidoData.direccionEnvioCodigoPostal);
                    if (pedidoData.direccionEnvioPoblacion) partes.push(pedidoData.direccionEnvioPoblacion);
                    if (pedidoData.direccionEnvioProvincia) partes.push(pedidoData.direccionEnvioProvincia);
                    if (pedidoData.direccionEnvioPais && pedidoData.direccionEnvioPais !== 'España') partes.push(pedidoData.direccionEnvioPais);
                    if (partes.length > 0) return partes.join(', ');
                    
                    // Fallback a dirección seleccionada o primera de envío
                    if (pedidoData.direccion) {
                      const dir = pedidoData.direccion;
                      const partesFallback = [];
          ...
          <div className="erp-form-group">
            ...
            <table className="erp-table erp-table-compact">
              <thead>
                <tr>
                  <th style={{ width: pedidoData.ventaMultialmacen ? '7%' : '8%' }}>Referencia</th>
                  <th style={{ width: pedidoData.ventaMultialmacen ? '22%' : '30%' }}>Producto</th>
                  {pedidoData.ventaMultialmacen && (
                    <th style={{ width: '13%' }}>Almacén</th>
                  )}
                  <th style={{ width: '7%', textAlign: 'center' }}>Cant.</th>
                  <th style={{ width: pedidoData.ventaMultialmacen ? '11%' : '14%', textAlign: 'center' }}>Precio</th>
                  <th style={{ width: '7%', textAlign: 'center' }}>Dto. %</th>
                  <th style={{ width: '7%', textAlign: 'center' }}>IVA %</th>
                  <th style={{ width: '7%', textAlign: 'center' }}>Rec. %</th>
                  <th style={{ width: pedidoData.ventaMultialmacen ? '11%' : '18%', textAlign: 'center' }}>Base</th>
                  <th style={{ width: '4%' }}></th>
                </tr>
              </thead>
              <tbody>
                {(pedidoData.lineas || []).map((linea, index) => (
                  <tr key={index}>
                ...
                {(!pedidoData.lineas || pedidoData.lineas.length === 0) && (
                  <tr>
                    <td colSpan={pedidoData.ventaMultialmacen ? 10 : 9} className="erp-td-empty">
          ...
          {/* Adjuntos */}
          {pedidoData.adjuntos && pedidoData.adjuntos.length > 0 && (
            <div className="erp-form-group">
              <h4 className="erp-form-group-title">Adjuntos</h4>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {pedidoData.adjuntos.map((adj, idx) => (
                  <button
                    key={idx}
                    type="button"
                    className="erp-btn erp-btn-secondary erp-btn-sm"
                    onClick={() => descargarAdjunto && descargarAdjunto(adj.id, adj.nombreArchivo)}
                  >
                    📎 {adj.nombreArchivo}
                  </button>
                ))}
