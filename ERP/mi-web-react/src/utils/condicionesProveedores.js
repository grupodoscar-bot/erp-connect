export const obtenerProveedorActivo = (proveedorId, proveedorConAgrupacion, proveedores) => {
  if (proveedorConAgrupacion) {
    return proveedorConAgrupacion;
  }
  return proveedores.find((p) => p.id === parseInt(proveedorId));
};

export const obtenerPrecioBaseProducto = (productoId, productos) => {
  const producto = productos.find((p) => p.id === parseInt(productoId));
  if (!producto) return 0;
  // Para compras, usar precioCompra o ultimoCoste
  return producto.precioCompra ?? producto.ultimoCoste ?? producto.precio ?? 0;
};
...
export const cargarCondicionesPorAgrupacion = async (agrupacionId, condicionesApiUrl) => {
  ...
  try {
    ...
    console.error("Error al cargar condiciones comerciales de proveedor:", err);
  }
...
export const aplicarCondicionComercialEnLinea = async ({
  ...
  const proveedorSeleccionado = obtenerProveedorActivo(proveedorId, proveedorConAgrupacion, proveedores);
  ...
  try {
    const res = await fetch(
      `${condicionesApiUrl}/agrupacion/${proveedorSeleccionado.agrupacion.id}/producto/${linea.productoId}`
    );
    ...
    console.error("Error al aplicar condición comercial de proveedor:", err);
  }
...
export const aplicarCondicionesProveedorEnLineas = async ({
  lineas,
  proveedorId,
  proveedorConAgrupacion,
  proveedores,
  ...
  if (!proveedorConAgrupacion?.agrupacion) {
    return lineas;
  }
