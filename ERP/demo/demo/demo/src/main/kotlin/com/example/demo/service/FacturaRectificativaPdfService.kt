...
@Service
class FacturaRectificativaPdfService(
    private val empresaRepository: EmpresaRepository,
    private val clienteRepository: ClienteRepository,
    private val direccionRepository: DireccionRepository
) {

    fun generarPdf(facturaRectificativa: FacturaRectificativa): ByteArray {
        ...
        // Título
        val titulo = Paragraph("FACTURA RECTIFICATIVA")
            .setFontSize(24f)
            .setBold()
        ...
        // Cliente
        val clienteInfo = facturaRectificativa.cliente?.let { cliente ->
        ...
        // Factura
        val facturaInfo = "Número: ${facturaRectificativa.numero}\n" +
            "Fecha: ${facturaRectificativa.fecha.format(DateTimeFormatter.ofPattern("dd/MM/yyyy"))}\n" +
            "Estado: ${facturaRectificativa.estado}"
        ...
        tablaTotales.addCell(Cell().add(Paragraph(String.format("%.2f €", facturaRectificativa.subtotal))).setTextAlignment(TextAlignment.RIGHT).setBorder(null))
        
        tablaTotales.addCell(Cell().add(Paragraph("Descuento:").setBold()).setTextAlignment(TextAlignment.RIGHT).setBorder(null))
        tablaTotales.addCell(Cell().add(Paragraph(String.format("%.2f €", facturaRectificativa.descuentoTotal))).setTextAlignment(TextAlignment.RIGHT).setBorder(null))
        ...
        tablaTotales.addCell(Cell().add(Paragraph(String.format("%.2f €", facturaRectificativa.total)).setBold().setFontSize(14f)).setTextAlignment(TextAlignment.RIGHT).setBorder(null))

        document.add(tablaTotales)

        // Observaciones
        if (!facturaRectificativa.observaciones.isNullOrBlank()) {
            document.add(Paragraph("\nObservaciones:").setBold().setMarginTop(20f))
            document.add(Paragraph(facturaRectificativa.observaciones).setFontSize(9f))
        }
