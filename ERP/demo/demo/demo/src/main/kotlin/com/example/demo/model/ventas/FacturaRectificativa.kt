...
@Entity
@Table(name = "ventas_facturas_rectificativas")
data class FacturaRectificativa(
    @Id
    ...
    @JoinColumn(name = "factura_origen_id")
    @JsonIgnoreProperties(value = ["hibernateLazyInitializer", "handler", "lineas"])
    val facturaOrigen: Factura? = null,

    @OneToMany(mappedBy = "facturaRectificativa", cascade = [CascadeType.ALL], orphanRemoval = true, fetch = FetchType.EAGER)
    @JsonManagedReference
    val lineas: MutableList<FacturaRectificativaLinea> = mutableListOf(),
