...
data class FacturaCompra(
    ...
    val pedidoCompra: PedidoCompra? = null,

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "albaran_compra_id")
    @JsonIgnoreProperties(value = ["hibernateLazyInitializer", "handler", "lineas"])
    val albaranCompra: AlbaranCompra? = null,

    @OneToMany(mappedBy = "facturaCompra", cascade = [CascadeType.ALL], orphanRemoval = true, fetch = FetchType.LAZY)
    @JsonManagedReference
    val lineas: MutableList<FacturaCompraLinea> = mutableListOf(),
